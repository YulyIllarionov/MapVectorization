...
std::vector<Wregion> WLine::CutFromLayer(WLayer* layer)
{
    ...
    point1 = vec2.СodirectionalVec(m_width).End();
    point2 = vec2.OppositeVec().СodirectionalVec(m_width).End();
    borderPoints.push_back(vec1.СodirectionalVec(m_width).End());
    borderPoints.push_back(vec2.СodirectionalVec(m_width).End());
    borderPoints.push_back(vec1.OppositeVec().СodirectionalVec(m_width).End());
    borderPoints.push_back(vec2.OppositeVec().СodirectionalVec(m_width).End());
    ...
    for (size_t i = 1; i < m_points.size() - 1; i++)
    {
        ...
        bisectVec = bisectVec.СodirectionalVec(m_width);
